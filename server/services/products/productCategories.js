...
class ProductCategoriesService {
	...
	async getCategories(params = {}) {
		...
		const items = await db.collection('productCategories').find(filter, { projection }).sort({ name: 1 }).toArray();
	...
	async addCategory(data) {
		const lastCategory = await db.collection('productCategories').findOne({}, { sort: { position: -1 } });
		...
		const insertResult = await db.collection('productCategories').insertMany([dataToInsert]);
	...
	updateCategory(id, data) {
		...
		return this.getValidDocumentForUpdate(id, data)
			.then((dataToSet) => db.collection('productCategories').updateOne({ _id: categoryObjectID }, { $set: dataToSet }))
			.then((res) => (res.modifiedCount > 0 ? this.getSingleCategory(id) : null));
	}

	async findAllChildren(items, id, result) {
		if (id && ObjectID.isValid(id)) {
			const parentItem = await getSingleCategory(id);
			result.push(new ObjectID(id));
			let finded = items.filter(
				(item) => (item.parent_category_id || '').toString() === parentItem.category_id.toString()
			);
			if (finded.length > 0) {
		...
		return this.getCategories()
			...
			.then((idsToDelete) => {
				...
				return db
					...
					.then((deleteResponse) => (deleteResponse.deletedCount > 0 ? idsToDelete : null));
			})
			.then((idsToDelete) => {
				...
				return idsToDelete
					? db
							.collection('products')
							.updateMany({ category_id: { $in: idsToDelete } }, { $set: { category_id: null } })
							.then(() => idsToDelete)
					: null;
			})
			.then((idsToDelete) => {
	...
	getValidDocumentForUpdate(id, data) {
		return new Promise((resolve, reject) => {
			...
			if (data.title !== undefined) {
				category.title = parse.getString(data.title);
			}
