...
class SitemapService {
	...
	getSlugArrayFromProducts(slug, onlyEnabled) {
		...
		// }
		return Promise.all([
			db.collection('productCategories').find(categoriesFilter).project({ slug: 1, products: 1 }).toArray(),
			db.collection('productItems').find(productFilter).project({ slug: 1, product_code: 1, sku: 1 }).toArray(),
		]).then(([categories, items]) => {
			return items.map((item) => {
				const category = categories.find((c) => {
					const products = get(c, 'products') || [];
					return products.includes(item.product_code);
				});
