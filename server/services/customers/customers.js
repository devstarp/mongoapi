...
class CustomersService {
	...
	getCustomers(params = {}) {
		...
		return Promise.all([
			...
			db.collection('customers').find(filter).sort({ date_created: -1 }).skip(offset).limit(limit).toArray(),
	...
	async addCustomer(data) {
		...
		if (customer.email && customer.email.length > 0) {
			const customerCount = await db.collection('customers').countDocuments({ email: customer.email });
			if (customerCount > 0) {
	...
	updateCustomerStatistics(customerId, totalSpent, ordersCount) {
		...
		return db.collection('customers').updateOne({ _id: customerObjectID }, { $set: customerData });
	}
	...
	async deleteCustomer(customerId) {
		...
		const deleteResponse = await db.collection('customers').deleteOne({ _id: customerObjectID });
		await webhooks.trigger({
	...
	getValidDocumentForUpdate(id, data) {
		...
		if (data.phone !== undefined) {
			customer.phone = parse.getString(data.phone);
		}
	...
	changeProperties(customer, customerGroups) {
		if (customer) {
			...
			if (customer.addresses && customer.addresses.length === 1) {
				...
				customer.billing = default_billing ? default_billing : customer.addresses[0];
				customer.shipping = default_shipping ? default_shipping : customer.addresses[0];
			} else {
				customer.billing = {};
				customer.shipping = {};
			}
	...
	createObjectToUpdateAddressFields(address) {
		...
		if (address.country !== undefined) {
			fields['addresses.$.country'] = parse.getObject(address.country);
		}

		if (address.state !== undefined) {
			fields['addresses.$.state'] = parse.getObject(address.state);
		}
		...
		if (address.email !== undefined) {
			fields['addresses.$.email'] = parse.getString(address.email);
		}
		if (address.first_name !== undefined) {
			fields['addresses.$.first_name'] = parse.getString(address.first_name);
		}

		if (address.last_name !== undefined) {
			fields['addresses.$.last_name'] = parse.getString(address.last_name);
		}
