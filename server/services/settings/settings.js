...
class SettingsService {
  constructor() {
    this.defaultSettings = {
      ...
      decimal_separator: '.',
      decimal_number: 2,
      timezone: 'USA/NewYork',
      date_format: 'MMMM D, YYYY',
      time_format: 'h:mm a',
      default_shipping_country: '',
      default_shipping_state: '',
      default_shipping_city: '',
      default_product_sorting: 'price,position',
      product_fields:
				'main_image,phase_out,saleable,product_code,available_quantity,ean,name,outlet,prices,sku,slug,id,position',
      products_limit: 30,
      weight_unit: 'kg',
      length_unit: 'cm',
  ...
  deleteLogo() {
    return this.getSettings().then((data) => {
      if (data.logo_file && data.logo_file.length > 0) {
        const filePath = path.resolve(settings.filesUploadPath + '/' + data.logo_file);
